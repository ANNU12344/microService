...
def search_blog(blog_id,post_id):
...
# sending the request to get all blog
# def get_all_blog():
#     Consumer_key = os.environ.get('TUMBLR_Consumer_Key')
#     url=f"https://api.tumblr.com/v2/blog/info"
#     headers={
#          'Consumer_key':Consumer_key 
#     }
#     app_logger.info(f'Sending request to get all blog')
#     response = requests.get(url, headers=headers)
#     app_logger.info(f'Shopify API Response Status Code: {response.status_code}')
#     if response.status_code == 200:
#             app_logger.info(f'Retrieved all blog')
#     elif response.status_code == 401:
#             app_logger.error('Unauthorized API call. Invalid API key or access token.')
#     return response

# # sending the request to get the blog with the specify blog_id
# def get_blog_by_id(blog_id):
#     Consumer_key = os.environ.get('TUMBLR_Consumer_Key')
#     print(f"blog_id:{blog_id},Consumer_key:{Consumer_key}")
#     url = f"https://api.tumblr.com/v2/blog/{blog_id}/info?api_key={Consumer_key}"
#     response = requests.get(url)
#     app_logger.info(f'Shopify API Response Status Code: {response.status_code}')
#     if response.status_code == 200:
#             app_logger.info(f'Retrieved info of blog with the help of blog id')
#     elif response.status_code == 401:
#             app_logger.error('Unauthorized API call. Invalid API key or access token.')
#     return response
